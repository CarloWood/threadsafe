...
void VoidPointerStorage::increase_size(uint32_t initial_size)
{
  ...
  m_free_indexes.consume_all([&tmp](index_type index){
    tmp.push_back(index);
  });
  m_free_indexes.reserve(m_size);

  ASSERT(m_free_indexes.empty());
...
//    Dout(dc::continued, ' ' << *iter);
    m_free_indexes.bounded_push(*iter);
  }
//  Dout(dc::finish, '.');
...
{
  // The storage contains no pointers when all indexes are free.
